...
func (cs *CalendarSuite) SetupSuite() {
	...
	grpcConnect, _ := grpc.NewClient(host+":"+port, grpc.WithTransportCredentials(insecure.NewCredentials()))
...
func (cs *CalendarSuite) insertTestEvent(ev *storage.Event) uuid.UUID {
	const query = `
		INSERT INTO events (title, date_time, duration, description, user_id, notification_time)
		VALUES ($1, $2, $3, $4, $5, $6)
		RETURNING id;
	`
	event := testEvent
